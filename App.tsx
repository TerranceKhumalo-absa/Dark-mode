
import React, { useState, useEffect } from 'react';
import { Moon, Sun, Info } from 'lucide-react';
import ButtonShowcase from './components/ButtonShowcase';
import TableShowcase from './components/TableShowcase';
import InputShowcase from './components/InputShowcase';
import PipelinesShowcase from './components/PipelinesShowcase';
import AlertShowcase from './components/AlertShowcase';
import BadgeShowcase from './components/BadgeShowcase';
import TabsShowcase from './components/TabsShowcase';
import DialogShowcase from './components/DialogShowcase';
import ButtonToggleShowcase from './components/ButtonToggleShowcase';
import PaletteViewer from './components/PaletteViewer';
import Sidebar from './components/Sidebar';
import { ViewState } from './types';

export type DarkThemeVariant = 'corporate' | 'contrast' | 'premium' | 'fresh' | 'scifi';

const App: React.FC = () => {
  const [currentView, setCurrentView] = useState<ViewState>('PIPELINES');
  const [isDarkMode, setIsDarkMode] = useState(true);
  const [darkTheme, setDarkTheme] = useState<DarkThemeVariant>('corporate');

  useEffect(() => {
    const html = document.documentElement;
    html.classList.remove('dark', 'light', 'theme-corporate', 'theme-contrast', 'theme-premium', 'theme-fresh', 'theme-scifi');
    if (isDarkMode) {
      html.classList.add('dark');
      html.classList.add(`theme-${darkTheme}`);
    } else {
      html.classList.add('light');
    }
  }, [isDarkMode, darkTheme]);

  const themes: { id: DarkThemeVariant; name: string; color: string }[] = [
    { id: 'corporate', name: 'Corporate', color: '#af144b' },
    { id: 'contrast', name: 'Contrast', color: '#f47721' },
    { id: 'premium', name: 'Premium', color: '#af144b' },
    { id: 'fresh', name: 'Fresh', color: '#f0325a' },
    { id: 'scifi', name: 'Sci-Fi', color: '#f52d28' },
  ];

  return (
    <div className="h-screen flex font-sans bg-absa-bg text-absa-text overflow-hidden">
      <aside className="shrink-0 z-20">
        <Sidebar currentView={currentView} setCurrentView={setCurrentView} isDarkMode={isDarkMode} />
      </aside>

      <div className="flex-1 flex flex-col h-full overflow-hidden">
        <header className="bg-white dark:bg-absa-surface border-b border-absa-border h-16 flex items-center justify-between px-8 shrink-0">
          <h2 className="text-lg font-bold text-absa-text">
            {currentView.charAt(0) + currentView.slice(1).toLowerCase().replace('_', ' ')}
          </h2>
          <div className="flex items-center gap-4">
            <button 
              onClick={() => setIsDarkMode(!isDarkMode)}
              className="flex items-center space-x-2 px-3 py-1.5 rounded-lg border border-transparent hover:border-absa-border bg-gray-100 dark:bg-black/20 text-gray-600 dark:text-absa-text"
            >
              {isDarkMode ? <Moon size={18} className="text-absa-primary" /> : <Sun size={18} className="text-orange-500" />}
              <span className="hidden sm:inline text-sm font-medium">{isDarkMode ? 'Dark Mode' : 'Light Mode'}</span>
            </button>
          </div>
        </header>

        {isDarkMode && (
          <div className="bg-absa-surface border-b border-absa-border py-3 px-8 shrink-0 overflow-x-auto">
            <div className="flex items-center gap-4 min-w-max">
              <div className="flex items-center gap-2 text-xs text-absa-muted uppercase tracking-wider font-bold">
                 <Info size={14} /> <span>Theme</span>
              </div>
              <div className="flex gap-3">
                {themes.map((theme) => (
                  <button
                    key={theme.id}
                    onClick={() => setDarkTheme(theme.id)}
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium border transition-all
                      ${darkTheme === theme.id ? 'bg-absa-primary/10 border-absa-primary text-absa-primary' : 'bg-transparent border-absa-border text-absa-muted hover:text-absa-text'}
                    `}
                  >
                    <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: theme.color }}></span>
                    {theme.name}
                  </button>
                ))}
              </div>
            </div>
          </div>
        )}

        <main className="flex-1 overflow-y-auto p-8">
          <div className="max-w-6xl mx-auto flex flex-col gap-8 pb-12">
            {currentView === 'PIPELINES' && <PipelinesShowcase />}
            {currentView === 'BUTTON' && <ButtonShowcase />}
            {currentView === 'BUTTON_TOGGLE' && <ButtonToggleShowcase />}
            {currentView === 'TABS' && <TabsShowcase />}
            {currentView === 'DIALOG' && <DialogShowcase />}
            {currentView === 'TABLE' && <TableShowcase />}
            {currentView === 'INPUT' && <InputShowcase />}
            {currentView === 'ALERTS' && <AlertShowcase />}
            {currentView === 'BADGES' && <BadgeShowcase />}
            {isDarkMode && <PaletteViewer theme={darkTheme} />}
          </div>
        </main>
      </div>
    </div>
  );
};

export default App;
